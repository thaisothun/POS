...
item_div.forEach((item)=>{
    item.addEventListener('click', (e)=>{
        ...
        item_cart.replaceChildren()
        cart.forEach((item)=>{
            rowHtml_cart_item = `<tr><td><img style="width: 52px; height: 52px;" src='${item.photo}'</td><td>${item.name}</td><td style="width: 80px;" ><input id="${item.item_id}" style="width: 70px;" type="number" value="${item.unit_price}" onclick="modify_price('${item.item_id}')"></td><td style=" text-align: center;" onclick="decrease_item('${item.item_id}')"><svg xmlns="http://www.w3.org/2000/svg" height="20px" viewBox="0 -960 960 960" width="20px" fill="#000000"><path d="M232-444v-72h496v72H232Z"/></svg></td><td style=" text-align: center;">${item.quantity}</td><td style=" text-align: center;" onclick="increase_item('${item.item_id}')"><svg xmlns="http://www.w3.org/2000/svg" height="20px" viewBox="0 -960 960 960" width="20px" fill="#000000"><path d="M444-444H240v-72h204v-204h72v204h204v72H516v204h-72v-204Z"/></svg></td><td style=" text-align: center;" onclick="remove_item('${item.item_id}')"><svg xmlns="http://www.w3.org/2000/svg" height="20px" viewBox="0 -960 960 960" width="20px" fill="#000000"><path d="M312-144q-29.7 0-50.85-21.15Q240-186.3 240-216v-480h-48v-72h192v-48h192v48h192v72h-48v479.57Q720-186 698.85-165T648-144H312Zm336-552H312v480h336v-480ZM384-288h72v-336h-72v336Zm12<PASSWORD> 0h72v-336h-72v336ZM312-696v48<PASSWORD>"/></svg></td></tr>`
            item_cart.insertAdjacentHTML("beforeend", rowHtml_cart_item);
        })
        const item_div = document.querySelectorAll('.item-card')
        const cart_el = document.getElementById('cart_save_item')
...
document.getElementById('customer_search').addEventListener('focusin',()=>{
    document.getElementById('customer_search_toggle').classList.toggle('show')
    const customer_serarch_div = document.getElementById('customer_search_toggle')
    customer_serarch_div.replaceChildren()
})

document.getElementById('customer_search').addEventListener('focusout',()=>{
    document.getElementById('customer_search_toggle').classList.toggle('show')})
...
search_customer_input.children[0].addEventListener('input', ()=>{
    if (search_customer_input.children[0].value =='') {
        const customer_serarch_div = document.getElementById('customer_search_toggle')
        customer_serarch_div.replaceChildren()} 
    else{
    ...
    async function send_get_data() {
        try {
            const response = await fetch('/sale/sale-transaction/search_customer/', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRFToken': getCookie('csrftoken') // Embedded directly in a Django template
            ...
            const customer_serarch_div = document.getElementById('customer_search_toggle')
            customer_serarch_div.replaceChildren()
            customer_serarch_div.insertAdjacentHTML("beforeend", '<ul id="customer_search_display"></ul>');
            const customer_serarch_div1 = document.getElementById('customer_search_display')
            if(data.length==0){
                rowHtml = `<li>Not Found</li>`
                customer_serarch_div1.insertAdjacentHTML("beforeend", rowHtml);
            } else{            
            data.forEach((item)=>{
...
form.addEventListener('submit', async function(e){
    ...
    try {
        const response = await fetch(`/sale/sale-transaction/add_customer/`, {
        method: 'POST',
        headers: {
            'X-CSRFToken': getCookie('csrftoken') // Embedded directly in a Django template
            },
        ...
        if (!response.ok) throw new Error('Network response was not ok');
            ...
            if (data['message_error']){
                Swal.fire({
                    ...
                    icon: "error"
                    }); window.addEventListener('click', ()=>{window.location.href = '/sale/sale-transaction/'})
            }
            if (data['form_error']){
                Swal.fire({
                    title: "Add Customer",
