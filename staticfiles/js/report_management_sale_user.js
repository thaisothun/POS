...
async function sale_transaction_report(start_date, end_date, path){
    const user = document.getElementById('selection').children[1].value
    if(!user){
         Swal.fire({
            title: "Sale Report By User",
            text: "User can not be blanked!",
            icon: "error"
            });
            return
        }
    ...
    data_send = {start_date : start_date.value, end_date : end_date.value, user:user}
    try {
        ...
        if (!response.ok) throw new Error('Network response was not ok');
            ...
            const rowHTML1 = `<tr><td></td><td></td><td></td><td></td><td>Total</td><td>${formatter.format(data['total_sale'])}</td><td></td><td>${formatter.format(data['total_discount'])}</td><td>${formatter.format(data['total_net_sale'])}</td><td></td><td></td><tr>`    
...
async function sale_transaction_report_excel(start_date, end_date, path){
    ...
    try {
        ...
        if (!response.ok) throw new Error('Network response was not ok');
            ...
            a.href = url;
            a.download = `sales_report_by_user_${now.toLocaleString()}.xlsx`;
...
document.getElementById('generate').addEventListener('click', ()=>{
    sale_transaction_report(start_date, end_date, '/report_management/generate-report/sale-by-user/generate/')})

document.getElementById('excel').addEventListener('click', ()=>{
    sale_transaction_report_excel(start_date, end_date, '/report_management/generate-report/sale-by-user/excel/')})
