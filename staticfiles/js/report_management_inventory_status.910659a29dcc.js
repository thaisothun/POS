...
start_date.addEventListener('change', ()=>{
    ...
    if (new Date(start_date.value) > current_date){
        Swal.fire({
            title: "Inventory Report By Status",
...
async function sale_transaction_report(start_date, end_date, path){
    ...
    const status = document.getElementById('selection').children[1].value
    data_send = {start_date : start_date.value, end_date : end_date.value, option:option,status:status}
    if (option != 'all'){   
    ...
    if(!status){
         Swal.fire({
            title: "Inventory Report By Status",
            text: "Status can not be blanked!",
            icon: "error"
            });
            return
        }
...
async function sale_transaction_report_excel(start_date, end_date, path){
    ...
    try {
        ...
        if (!response.ok) throw new Error('Network response was not ok');
            ...
            a.href = url;
            a.download = `inventory_report_by_status_${now.toLocaleString()}.xlsx`;
...
document.getElementById('generate').addEventListener('click', ()=>{
    sale_transaction_report(start_date, end_date, '/report_management/generate-report/inventory-status/generate/')})

document.getElementById('excel').addEventListener('click', ()=>{
    sale_transaction_report_excel(start_date, end_date, '/report_management/generate-report/inventory-status/excel/')})
