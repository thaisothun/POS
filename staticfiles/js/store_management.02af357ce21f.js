...
async function category(data) {
    ...
    if (search.value == ''){
        ...
        try {
            const response = await fetch('/store-management/sort/', {
            method: 'POST',
            headers: {
        ...
        try {
            const response = await fetch('/store-management/search/', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRFToken': getCookie('csrftoken') // Embedded directly in a Django template
...
async function get_store(div_name, id) {
    show_div(div_name)
    const data_send = {id: id}
    try {
        const response = await fetch('/store-management/get/', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        ...
        if (!response.ok) throw new Error('Network response was not ok');
            const data = await response.json();
            data.forEach((item)=>{
                const input = document.querySelectorAll('#store_modify div')
                document.getElementById('category_id').innerHTML = item.id
                input[0].children[1].value = item.name
                input[1].children[1].value = item.code
                input[2].children[1].value = item.phone
                input[3].children[1].value = item.address
                input[4].children[1].value = item.sale_target
                input[5].children[0].src = `/media/${item.logo}`
            })           
    } catch (error) {
        console.error('Fetch error:', error);
    }
...
form.addEventListener('submit', async function(e){
    ...
    try {
        const response = await fetch(`/store-management/modify/${id}/`, {
        method: 'POST',
        headers: {
            'X-CSRFToken': getCookie('csrftoken') // Embedded directly in a Django template
            },
        ...
        if (!response.ok) throw new Error('Network response was not ok');
            ...
            if (data['message_sucess']){
                Swal.fire({
                    ...
                    icon: "success"
                    });  window.addEventListener('click', ()=>{window.location.href = '/store-management/'})
            }
