...
async function sale_transaction_filter_date(){
    ...
    try {
        const response = await fetch(`/purchase-transaction/purchase-transaction-list/filtter-date/approval/`, {
        method: 'POST',
        headers: {
        ...
        if (!response.ok) throw new Error('Network response was not ok');
            ...
            data.forEach(item=>{  
                const rowHTML = `<tr><td>${new Date(item.transaction_date).toLocaleString('en-us',option).replace(' at ', ', ')}</td><td>${item.store__name}</td><td>${item.invoice_number}</td><td>${item.user}</td><td>${item.full_name}</td><td>${item.total_amount}</td><td>${item.promotion_code__name}</td><td>${item.discount}</td><td>${item.net_amount}</td><td>${item.payment_method}</td><td>${item.status}</td><td>
                <a title="Approved Delete ${item.invoice_number}" href="javascript:void(0)" id="print" onclick="show_div('wrapper_delete_item','${item.invoice_number}')"><svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#000000"><path d="M564-80 394-250l56-56 114 114 226-226 56 56L564-80ZM120-320l194-520h94l194 520h-92l-46-132H254l-46 132h-88Zm162-208h156l-76-216h-4l-76 216Z"/></svg></a>
                <a title="Rejected Delete ${item.invoice_number}" href="javascript:void(0)" id="delete" onclick="show_div_invoice('wrapper_delete_reject','${item.invoice_number}')"><svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#000000"><path d="m336-280 144-144 144 144 56-56-144-144 144-144-56-56-144 144-144-144-56 56 144 144-144 144 56 56ZM480-80q-83 0-156-31.5T197-197q-54-54-85.5-127T80-480q0-83 31.5-156T197-763q54-54 127-85.5T480-880q83 0 156 31.5T763-763q54 54 85.5 127T880-480q0 83-31.5 156T763-197q-54 54-127 85.5T480-80Zm0-80q134 0 227-93t93-227q0-134-93-227t-227-93q-134 0-227 93t-93 227q0 134 93 227t227 93Zm0-320Z"/></svg></a>
                </td></tr>`;
...
date_from.addEventListener('change', ()=>{
    ...
    if (new Date(date_from.value) > current_date){
        Swal.fire({
            title: "Purchase Transaction Filter",
...
async function sale_transaction(data) {
    ...
    if (search.value == ''){
        ...
        try {
            const response = await fetch('/purchase-transaction/purchase-transaction-list/sort/approval/', {
            method: 'POST',
            headers: {
        ...
        try {
            const response = await fetch('/purchase-transaction/purchase-transaction-list/search/approval/', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRFToken': getCookie('csrftoken') // Embedded directly in a Django template
...
form_delete_invoice.addEventListener('submit', async function(e){
    ...
    try {
        const response = await fetch(`/purchase-transaction/purchase-transaction-list/delete/approval/${id}/`, {
        method: 'POST',
        headers: {
            'X-CSRFToken': getCookie('csrftoken') // Embedded directly in a Django template
            },
...
form_delete_invoice_reject.addEventListener('submit', async function(e){
    ...
    id = item_id.innerHTML
    try {
        const response = await fetch(`/purchase-transaction/purchase-transaction-list/delete/reject/${id}/`, {
        method: 'POST',
        headers: {
            'X-CSRFToken': getCookie('csrftoken') // Embedded directly in a Django template
            },
