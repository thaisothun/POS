...
async function customer(data) {
    ...
    if (search.value == ''){
        ...
        try {
            const response = await fetch('/supplier-management/deactivate-sort/', {
            method: 'POST',
            headers: {
        ...
        try {
            const response = await fetch('/supplier-management/deactivate-search/', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRFToken': getCookie('csrftoken') // Embedded directly in a Django template
...
form_activate.addEventListener('submit', async function(e){
    ...
    try {
        const response = await fetch(`/supplier-management/activate/${id}/`, {
        method: 'POST',
        headers: {
            'X-CSRFToken': getCookie('csrftoken') // Embedded directly in a Django template
            },
        ...
        if (!response.ok) throw new Error('Network response was not ok');
            ...
            if (data.message_sucess){
                Swal.fire({
                    ...
                    icon: "success"
                    }); window.addEventListener('click', ()=>{window.location.href = '/people/supplier-deactivate-list/'})
            }
