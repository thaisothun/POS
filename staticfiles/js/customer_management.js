...
async function customer(data) {
    ...
    if (search.value == ''){
        ...
        try {
            const response = await fetch('/customer_management/sort/', {
            method: 'POST',
            headers: {
        ...
        try {
            const response = await fetch('/customer_management/search/', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRFToken': getCookie('csrftoken') // Embedded directly in a Django template
...
form_deactivate.addEventListener('submit', async function(e){
    ...
    try {
        const response = await fetch(`/customer_management/deactivate/${id}/`, {
        method: 'POST',
        headers: {
            'X-CSRFToken': getCookie('csrftoken') // Embedded directly in a Django template
            },
        ...
        if (!response.ok) throw new Error('Network response was not ok');
            ...
            if (data.message_sucess){
                Swal.fire({
                    ...
                    icon: "success"
                    }); window.addEventListener('click', ()=>{window.location.href = '/people/customer-list/'})
            }
