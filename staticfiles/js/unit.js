...
async function unit(data) {
    ...
    if (search.value == ''){
        ...
        try {
            const response = await fetch('/unit/sort/', {
            method: 'POST',
            headers: {
        ...
        try {
            const response = await fetch('/unit/search/', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRFToken': getCookie('csrftoken') // Embedded directly in a Django template
...
async function get_unit(div_name, product_id) {
    show_div(div_name)
    const data_send = {product_id: product_id}
    try {
        const response = await fetch('/unit/get/', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        ...
        if (!response.ok) throw new Error('Network response was not ok');
            const data = await response.json();
            data.forEach(item=>{
            const item_id = document.getElementById('unit_id')    
...
form.addEventListener('submit', async function(e){
    ...
    try {
        const response = await fetch(`/unit/modify/${id}/`, {
        method: 'POST',
        headers: {
            'X-CSRFToken': getCookie('csrftoken') // Embedded directly in a Django template
            },
        ...
        if (!response.ok) throw new Error('Network response was not ok');
            ...
            if (data.__all__){
                Swal.fire({
                    ...
                    text: data.__all__,
                    icon: "error"
                    }); window.addEventListener('click', ()=>{window.location.href = '/inventory/unit/'})
            }
            if (data.name){
                Swal.fire({
                    title: "Update Item",
